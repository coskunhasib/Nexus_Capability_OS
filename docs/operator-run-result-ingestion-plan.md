...
The goal is to accept operator-managed runtime result files and normalize them through the existing runtime adapter response path before any direct runtime mode is considered.
...
The system can already prepare safe work envelopes and show operator-facing runtime state.

The missing bridge is:

```text
operator runs an external tool outside the app
operator provides a result file
Nexus validates the result file
Nexus normalizes it to runtime adapter response/events
```

This proves the result path before any direct external runtime wiring.
...
→ normalize with existing adapter normalizer
...
```text
add a Code Agent operator-run result type
include agent_kind in the result file
add completed and blocked fixtures for supported agent kinds
add a verifier that normalizes fixtures
reuse normalizeCodeAgentResult
keep direct runtime mode disabled
```
...
Future PR 36 should decide the first real external runtime path.
...
```text
verify:openhands-result-ingest
