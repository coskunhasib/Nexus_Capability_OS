...
- **Phase:** G/H (doc 21 — [`../21-runtime-refactor-planning-only-plan.md`](../21-runtime-refactor-planning-only-plan.md), §4 "Pipeline run state model plan"; §5 required output `runtime-refactor-plan/pipeline-run-state-model.md`)
- **Models:** the **07 Pipeline Run Contract** = [`../07-pipeline-contract-standard.md`](../07-pipeline-contract-standard.md) §3 (the 25-field Pipeline Run shape) — D-009 Pipeline != Pipeline Run.
...
- **Status of this layer:** PLAN ONLY. No runtime code, no product refactor, no connector integration, no deploy/publish, no main-branch merge, no plugin registry. Transition to any implementation is gated by [`../22-final-stop-rule-and-readiness-gate.md`](../22-final-stop-rule-and-readiness-gate.md) plus an explicit **owner decision** (see §11).
...
| --- | --- |
| Run-state field set (25 fields) | [`../07-pipeline-contract-standard.md`](../07-pipeline-contract-standard.md) §3; [`../../../configs/nexus-ai/pipeline_contract_standard.yaml`](../../../configs/nexus-ai/pipeline_contract_standard.yaml) `pipeline_run_required_fields` |
...
| Stage set + ordering + per-stage on_pass/on_fail/rework_route | [`../../../configs/nexus-ai/stages/sdlc/*.yaml`](../../../configs/nexus-ai/stages/sdlc/) (31 stage contracts); schema [`stage_contract.schema.json`](../../../configs/nexus-ai/schemas/stage_contract.schema.json) |
...
| Locked decisions enforced in-state | [`../00-decision-log.md`](../00-decision-log.md) (D-001…D-010); schema `$comment` locks K-001/K-003/K-004/K-008 |
