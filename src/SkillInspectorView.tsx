...
function StepList({ items }: { items: string[] }) {
  if (!items.length) return <span className="text-xs text-neutral-500">none</span>;
  return <div className="space-y-2">{items.map((item, index) => <div key={`${index}:${item}`} className="rounded-xl border border-white/10 bg-black/30 p-4 text-sm leading-relaxed text-neutral-300"><Badge tone="cyan">{String(index + 1).padStart(2, '0')}</Badge><span className="ml-3">{item}</span></div>)}</div>;
}
...
export default function SkillInspectorView() {
  ...
  return <div className="min-h-screen bg-[#050505] px-4 py-8 text-neutral-200 sm:px-6"><div className="mx-auto flex max-w-[1500px] flex-col gap-6"><header className="rounded-2xl border border-white/10 bg-[#0a0a0a] p-6 shadow-2xl"><div className="flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-cyan-500 text-black"><BrainCircuit size={26} strokeWidth={2.5} /></div><div><h1 className="text-2xl font-semibold tracking-tight text-white">Skill Inspector</h1><p className="mt-1 text-sm text-neutral-400">Canonical skills are now first-class runtime ingredients, not loose labels.</p></div></div><div className="mt-5 flex flex-wrap gap-2"><Badge tone="cyan">{skills.length} skills</Badge><Badge tone="green">{packs.length} packs</Badge><Badge tone="yellow">audit enforced</Badge></div></header>
    <section className="grid gap-6 xl:grid-cols-[390px_1fr]"><aside className="rounded-2xl border border-white/10 bg-[#0a0a0a] p-5"><label className="mb-3 flex items-center gap-2 text-sm font-semibold text-white"><Search size={18} className="text-cyan-400" />Search skills</label><input value={query} onChange={(event) => setQuery(event.target.value)} className="mb-4 w-full rounded-xl border border-white/10 bg-black/50 px-4 py-3 text-sm text-neutral-200 outline-none transition focus:border-cyan-500/50" placeholder="Search skill, status, pack..." /><div className="mb-3 text-xs font-bold uppercase tracking-widest text-neutral-500">{filtered.length} skills</div><div className="space-y-2">{filtered.map((skill) => <button key={skill.id} onClick={() => setSelectedId(skill.id)} className={`w-full rounded-xl border p-3 text-left transition ${selected?.id === skill.id ? 'border-cyan-500/40 bg-cyan-950/20' : 'border-white/10 bg-black/30 hover:bg-white/5'}`}><div className="flex items-center justify-between gap-2"><div className="truncate text-sm font-medium text-white">{skill.id}</div><Badge tone={skill.status === 'active' ? 'green' : skill.status === 'deprecated' ? 'red' : 'cyan'}>{skill.status}</Badge></div><p className="mt-2 line-clamp-3 text-xs leading-relaxed text-neutral-400">{skill.summary}</p><div className="mt-3 flex flex-wrap gap-1">{skill.quality_gates.slice(0, 3).map((gate) => <span key={gate} className="rounded border border-white/10 px-1.5 py-0.5 text-[10px] text-neutral-400">{gate}</span>)}</div></button>)}</div></aside>
    {selected && <main className="flex flex-col gap-6"><section className="rounded-2xl border border-white/10 bg-[#0a0a0a] p-6"><div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between"><div><div className="mb-3 flex flex-wrap gap-2"><Badge tone={statusTone}>{selected.status}</Badge><Badge>{selected.source_lineage.source_type}</Badge><Badge tone="yellow">{linkedPacks.length} linked packs</Badge></div><h2 className="text-3xl font-light text-white">{selected.title}</h2><p className="mt-4 max-w-4xl text-sm leading-relaxed text-neutral-300">{selected.summary}</p></div><div className="rounded-xl border border-white/10 bg-black/40 p-4 text-xs leading-relaxed text-neutral-400 lg:w-[360px]"><div className="mb-2 text-[10px] font-bold uppercase tracking-widest text-neutral-500">source lineage</div><div className="text-white">{selected.source_lineage.origin}</div><p className="mt-2">{selected.source_lineage.notes}</p></div></div></section>
