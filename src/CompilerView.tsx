...
const defaultIntent = 'Build a web SaaS MVP with React frontend, backend API, database, tests and release gates.';
...
function readInitialIntent() {
  if (typeof window === 'undefined') return defaultIntent;
  const pending = window.localStorage.getItem(pendingTrialIntentKey);
  if (pending) {
    window.localStorage.removeItem(pendingTrialIntentKey);
    return pending;
  }
...
export default function CompilerView({ onSendTaskPacket }: { onSendTaskPacket?: (packet: unknown) => void }) {
  const [intent, setIntent] = useState(readInitialIntent);
