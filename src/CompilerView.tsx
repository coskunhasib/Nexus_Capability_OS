...
import { Boxes, BrainCircuit, CheckCircle2, FileText, Layers, Network, Search, ShieldCheck, Users } from 'lucide-react';
...
const agentProfiles = (agentProfilesData as { agent_profiles: AgentProfile[] }).agent_profiles ?? [];
const gates = (gatesData as { gates: Gate[] }).gates ?? [];
const microPipelines = (microPipelinesData as { micro_pipelines: MicroPipeline[] }).micro_pipelines ?? [];
const packs = (packsData as { capability_packs: CapabilityPack[] }).capability_packs ?? [];

function toMap<T extends { id?: string; pack_id?: string }>(items: T[], key: 'id' | 'pack_id' = 'id') {
  return new Map(items.map((item) => [item[key] as string, item]));
}
...
function getUnresolvedIds(selected?: Rule) {
  ...
  for (const id of selected.select.profiles ?? []) {
    if (!profileMap.has(id)) unresolved.push(`profile:${id}`);
  }
  for (const id of selected.select.gates ?? []) {
    if (!gateMap.has(id)) unresolved.push(`gate:${id}`);
  }
  for (const id of selected.select.micro_pipelines ?? []) {
    if (!microPipelineMap.has(id)) unresolved.push(`micro:${id}`);
  }
  for (const id of selected.select.capability_packs ?? []) {
    if (!packMap.has(id)) unresolved.push(`pack:${id}`);
  }
...
function ListLine({ label, items }: { label: string; items?: string[] }) {
  ...
  return (
    <div className="mt-3">
      <div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-neutral-500">{label}</div>
...
function DetailCard({ title, subtitle, summary, badges, children }: { title: string; subtitle?: string; summary?: string; badges?: ReactNode; children?: ReactNode }) {
  return (
    <div className="rounded-xl border border-white/10 bg-[#0e0e0e] p-5">
      ...
      {summary && <p className="mt-3 text-sm leading-relaxed text-neutral-400">{summary}</p>}
      {children}
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#050505] text-neutral-200 selection:bg-cyan-500/30 selection:text-cyan-200">
      <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-6 px-4 py-8 sm:px-6">
        ...
        <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="rounded-2xl border border-white/10 bg-[#0a0a0a] p-6">
            <div className="mb-3 text-xs font-bold uppercase tracking-widest text-neutral-500">Execution brief</div>
            {selected ? (
              <div>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  ...
                  <div className="rounded-xl border border-white/10 bg-black/40 p-4">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-neutral-500">Memory Policy</div>
                    <div className="mt-2 text-sm text-white">{selected.select?.memory_policy ?? 'none'}</div>
                  </div>
                  <div className="rounded-xl border border-white/10 bg-black/40 p-4">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-neutral-500">Context Policy</div>
                    <div className="mt-2 text-sm text-white">{selected.select?.context_policy ?? 'none'}</div>
                  </div>
                </div>
              </div>
            ) : (
        ...
        {selected && (
          <>
            <Section icon={<Network size={18} />} title="Micro pipeline cards">
              <div className="grid gap-4 lg:grid-cols-2">
                {selectedMicroPipelines.map((pipeline) => (
                  <DetailCard key={pipeline.id} title={pipeline.title} subtitle={pipeline.id} summary={pipeline.summary} badges={<Badge tone="cyan">{pipeline.parent_macro}</Badge>}>
                    <ListLine label="required profiles" items={pipeline.required_profiles} />
                    <ListLine label="required gates" items={pipeline.required_gates} />
                    <ListLine label="outputs" items={pipeline.outputs} />
                  </DetailCard>
                ))}
              </div>
            </Section>

            <Section icon={<Users size={18} />} title="Team profile cards">
              <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
                {selectedProfiles.map((profile) => (
                  <DetailCard key={profile.id} title={profile.title} subtitle={profile.id} summary={profile.summary} badges={<Badge>{profile.family}</Badge>}>
                    <ListLine label="does" items={profile.does} />
                    <ListLine label="outputs" items={profile.outputs} />
                    <ListLine label="failure modes" items={profile.failure_modes} />
                  </DetailCard>
                ))}
              </div>
            </Section>

            <Section icon={<ShieldCheck size={18} />} title="Gate cards">
              <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
                {selectedGates.map((gate) => {
                  const severityTone = gate.severity === 'critical' ? 'red' : gate.severity === 'blocking' ? 'yellow' : 'neutral';
                  return (
                    <DetailCard key={gate.id} title={gate.title} subtitle={gate.id} summary={gate.summary} badges={<Badge tone={severityTone}>{gate.severity ?? 'gate'}</Badge>}>
                      <ListLine label="pass criteria" items={gate.pass_criteria} />
                      <ListLine label="fail criteria" items={gate.fail_criteria} />
                    </DetailCard>
                  );
                })}
              </div>
            ...
            <Section icon={<Boxes size={18} />} title="Capability pack details">
              <div className="grid gap-4 lg:grid-cols-2">
                {selectedPacks.map((pack) => (
                  <DetailCard key={pack.pack_id} title={pack.pack_id} subtitle={pack.status} summary={pack.summary} badges={<Badge tone="emerald">{pack.macro_pipeline}</Badge>}>
                    <ListLine label="intents" items={pack.for_intents} />
                    <ListLine label="micro pipelines" items={pack.micro_pipelines} />
                    <ListLine label="profiles" items={pack.profiles} />
                    <ListLine label="skills" items={pack.skills} />
                    <ListLine label="tools" items={pack.tools} />
                    <ListLine label="quality gates" items={pack.quality_gates} />
                  </DetailCard>
                ))}
              </div>
            </Section>
            ...
            {unresolvedIds.length > 0 && (
              <Section icon={<FileText size={18} />} title="Missing / unresolved registry IDs">
                <div className="flex flex-wrap gap-2">
                  {unresolvedIds.map((id) => <Badge key={id} tone="red">{id}</Badge>)}
                </div>
              </Section>
            )}
          </>
