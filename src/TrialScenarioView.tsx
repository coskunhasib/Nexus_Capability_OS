...
function Badge({ children, tone = 'neutral' }: { children: string | number; tone?: 'neutral' | 'cyan' | 'green' | 'yellow' }) {
  const cls = {
...
export default function TrialScenarioView({ onOpenCompiler }: { onOpenCompiler?: () => void }) {
  ...
  const copyAndOpenCompiler = async () => {
    await copyIntent();
    onOpenCompiler?.();
  };
