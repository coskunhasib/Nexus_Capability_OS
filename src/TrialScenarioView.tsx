...
import webSaasRuntime from '../samples/runtime-bridge-results/web-saas-mvp/runtime-bridge-summary.json';
import agenticRuntime from '../samples/runtime-bridge-results/agentic-system/runtime-bridge-summary.json';
import stm32Runtime from '../samples/runtime-bridge-results/stm32-firmware/runtime-bridge-summary.json';
import rfqRuntime from '../samples/runtime-bridge-results/rfq-generation/runtime-bridge-summary.json';
import technicalReportRuntime from '../samples/runtime-bridge-results/technical-report/runtime-bridge-summary.json';
...
type RuntimeBridgeResult = {
  ...
  event_count: number;
  event_types: string[];
  payload_coverage: Record<string, boolean>;
};
...
const runtimeResults = [webSaasRuntime, agenticRuntime, stm32Runtime, rfqRuntime, technicalReportRuntime] as RuntimeBridgeResult[];
...
const runtimeMap = new Map(runtimeResults.map((result) => [result.scenario_id, result]));
...
function RuntimeBridgeCard({ result }: { result?: RuntimeBridgeResult }) {
  if (!result) return <div className="rounded-xl border border-red-500/20 bg-red-950/10 p-4 text-sm text-red-100">No runtime bridge snapshot found.</div>;
  const failedAssertions = result.assertions.filter((assertion) => !assertion.pass);
  const coverage = Object.entries(result.payload_coverage);
  return <div className="rounded-xl border border-white/10 bg-black/30 p-5"><div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><div className="mb-2 flex flex-wrap gap-2"><Badge tone={result.status === 'pass' ? 'green' : 'red'}>{result.status}</Badge><Badge>{result.trial_id}</Badge><Badge tone="cyan">{result.event_count} events</Badge></div><div className="text-sm text-neutral-400">Nexus runtime bridge snapshot: callback events and payload coverage from mock worker.</div></div><div className="grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-lg border border-white/10 bg-[#050505] p-2"><div className="text-lg text-white">{result.event_count}</div><div className="text-neutral-500">events</div></div><div className="rounded-lg border border-white/10 bg-[#050505] p-2"><div className="text-lg text-white">{result.event_types.length}</div><div className="text-neutral-500">types</div></div><div className="rounded-lg border border-white/10 bg-[#050505] p-2"><div className="text-lg text-white">{coverage.filter(([, pass]) => pass).length}</div><div className="text-neutral-500">payloads</div></div></div></div><div className="mb-4"><div className="mb-2 text-[10px] font-bold uppercase tracking-widest text-neutral-500">event types</div><PillList items={result.event_types} tone="cyan" /></div><div className="mb-4"><div className="mb-2 text-[10px] font-bold uppercase tracking-widest text-neutral-500">payload coverage</div><div className="flex flex-wrap gap-2">{coverage.map(([name, pass]) => <Badge key={name} tone={pass ? 'green' : 'red'}>{name}: {pass ? 'covered' : 'missing'}</Badge>)}</div></div><AssertionGrid assertions={result.assertions} />{failedAssertions.length > 0 && <div className="mt-4 rounded-lg border border-red-500/20 bg-red-950/10 p-3 text-xs text-red-100">{failedAssertions.length} runtime bridge assertions need investigation.</div>}</div>;
}
...
export default function TrialScenarioView({ onOpenCompiler }: { onOpenCompiler?: () => void }) {
  ...
  const selectedRuntime = runtimeMap.get(selected.scenario_id);
  ...
  const runtimePassCount = runtimeResults.filter((result) => result.status === 'pass').length;
  ...
  return <div className="min-h-screen bg-[#050505] px-4 py-8 text-neutral-200 sm:px-6"><div className="mx-auto flex max-w-[1500px] flex-col gap-6"><header className="rounded-2xl border border-white/10 bg-[#0a0a0a] p-6 shadow-2xl"><div className="flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-cyan-500 text-black"><FlaskConical size={26} strokeWidth={2.5} /></div><div><h1 className="text-2xl font-semibold tracking-tight text-white">Trial Scenario Library</h1><p className="mt-1 text-sm text-neutral-400">Repeatable test cases, CI-backed skill-aware trial results, Nexus handoff usability and runtime bridge coverage.</p></div></div><div className="mt-5 flex flex-wrap gap-2"><Badge tone="cyan">{scenarios.length} scenarios</Badge><Badge tone={suitePassCount === results.length ? 'green' : 'red'}>{suitePassCount}/{results.length} trials passing</Badge><Badge tone={handoffPassCount === handoffResults.length ? 'green' : 'red'}>{handoffPassCount}/{handoffResults.length} handoffs usable</Badge><Badge tone={runtimePassCount === runtimeResults.length ? 'green' : 'red'}>{runtimePassCount}/{runtimeResults.length} runtime bridges passing</Badge><Badge tone="yellow">skill-aware CI snapshots</Badge></div></header>
    <Panel icon={<ShieldCheck size={18} />} title="CI Trial + Handoff + Runtime Dashboard"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">{scenarios.map((scenario) => { const result = resultMap.get(scenario.scenario_id); const handoff = handoffMap.get(scenario.scenario_id); const runtime = runtimeMap.get(scenario.scenario_id); return <button key={scenario.scenario_id} onClick={() => setSelectedId(scenario.scenario_id)} className={`rounded-xl border p-4 text-left transition ${selected.scenario_id === scenario.scenario_id ? 'border-cyan-500/40 bg-cyan-950/20' : 'border-white/10 bg-black/30 hover:bg-white/5'}`}><div className="mb-2 flex items-center justify-between gap-2"><span className="text-sm font-semibold text-white">{scenario.scenario_id}</span><div className="flex flex-wrap gap-1"><Badge tone={result?.status === 'pass' ? 'green' : 'red'}>{result?.status ?? 'missing'}</Badge><Badge tone={handoff?.status === 'pass' ? 'green' : 'red'}>handoff</Badge><Badge tone={runtime?.status === 'pass' ? 'green' : 'red'}>runtime</Badge></div></div><div className="grid grid-cols-3 gap-1 text-center text-[10px] text-neutral-500"><div className="rounded border border-white/10 bg-[#050505] p-1"><div className="text-neutral-200">{result?.report_summary.missing_evidence ?? '-'}</div>miss</div><div className="rounded border border-white/10 bg-[#050505] p-1"><div className="text-neutral-200">{handoff?.handoff_summary.required_tool_count ?? '-'}</div>tools</div><div className="rounded border border-white/10 bg-[#050505] p-1"><div className="text-neutral-200">{runtime?.event_count ?? '-'}</div>events</div></div></button>; })}</div></Panel>
    <section className="grid gap-6 xl:grid-cols-[380px_1fr]"><aside className="rounded-2xl border border-white/10 bg-[#0a0a0a] p-5"><label className="mb-3 flex items-center gap-2 text-sm font-semibold text-white"><Search size={18} className="text-cyan-400" />Search trials</label><input value={query} onChange={(event) => setQuery(event.target.value)} className="mb-4 w-full rounded-xl border border-white/10 bg-black/50 px-4 py-3 text-sm text-neutral-200 outline-none focus:border-cyan-500/50" placeholder="Search intent, tag, scenario..." /><div className="mb-3 text-xs font-bold uppercase tracking-widest text-neutral-500">{filtered.length} scenarios</div><div className="space-y-2">{filtered.map((scenario) => { const result = resultMap.get(scenario.scenario_id); const handoff = handoffMap.get(scenario.scenario_id); const runtime = runtimeMap.get(scenario.scenario_id); return <button key={scenario.scenario_id} onClick={() => setSelectedId(scenario.scenario_id)} className={`w-full rounded-xl border p-3 text-left transition ${selected.scenario_id === scenario.scenario_id ? 'border-cyan-500/40 bg-cyan-950/20' : 'border-white/10 bg-black/30 hover:bg-white/5'}`}><div className="flex items-center justify-between gap-2"><div className="text-sm font-medium text-white">{scenario.scenario_id}</div><div className="flex gap-2"><Badge tone={result?.status === 'pass' ? 'green' : 'red'}>{result?.status ?? 'missing'}</Badge><Badge tone={handoff?.status === 'pass' ? 'green' : 'red'}>handoff</Badge><Badge tone={runtime?.status === 'pass' ? 'green' : 'red'}>runtime</Badge><Badge tone="cyan">v{scenario.version}</Badge></div></div><p className="mt-2 line-clamp-3 text-xs leading-relaxed text-neutral-400">{scenario.intent}</p><div className="mt-3 flex flex-wrap gap-1">{(scenario.tags ?? []).slice(0, 4).map((tag) => <span key={tag} className="rounded border border-white/10 px-1.5 py-0.5 text-[10px] text-neutral-400">{tag}</span>)}</div></button>; })}</div></aside>
    {selected && <main className="flex flex-col gap-6"><Panel icon={<ShieldCheck size={18} />} title="Selected trial CI result"><ResultCard result={selectedResult} /></Panel><Panel icon={<ShieldCheck size={18} />} title="Selected Nexus handoff usability"><HandoffCard result={selectedHandoff} /></Panel><Panel icon={<ShieldCheck size={18} />} title="Selected Nexus runtime bridge"><RuntimeBridgeCard result={selectedRuntime} /></Panel><Panel icon={<Target size={18} />} title="Trial intent"><div className="rounded-xl border border-white/10 bg-black/30 p-5"><div className="mb-3 flex flex-wrap gap-2"><Badge tone="cyan">{selected.scenario_id}</Badge><Badge>{selected.expected.macro_pipeline}</Badge><Badge tone="green">{selected.expected.capability_pack}</Badge></div><p className="text-lg leading-relaxed text-white">{selected.intent}</p><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => void copyIntent()} className="flex items-center gap-2 rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs font-semibold text-neutral-300 hover:text-white"><Clipboard size={14} />Copy intent</button><button onClick={() => void openInCompiler()} className="flex items-center gap-2 rounded-lg border border-cyan-500/20 bg-cyan-950/20 px-3 py-2 text-xs font-semibold text-cyan-300 hover:bg-cyan-900/30"><Clipboard size={14} />Load in Compiler</button></div></div></Panel>
