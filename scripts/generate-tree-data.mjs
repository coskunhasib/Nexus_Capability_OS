...
const treeData = node('root', 'Root', 'Nexus Capability OS', 'Registry-generated capability map. src/data.ts is generated from registry/*.json.', {
...
const output = `// AUTO-GENERATED by scripts/generate-tree-data.mjs. Do not edit by hand.\nexport type NodeType = 'Root' | 'Architecture' | 'Macro Pipeline' | 'Micro Pipeline' | 'Profile Family' | 'Layer' | 'Skill' | 'Tool' | 'Runtime' | 'Memory/Context' | 'Gate' | 'Stage';\n\nexport interface TreeNode {\n  id: string;\n  type: NodeType;\n  title: string;\n  mini?: string;\n  desc?: string;\n  bullets?: string[];\n  tags?: string[];\n  gates?: string[];\n  children?: TreeNode[];\n}\n\nexport const treeData: TreeNode = ${JSON.stringify(treeData, null, 2)};\n`;

fs.writeFileSync(path.join(srcDir, 'generated-tree-data.ts'), output);
fs.writeFileSync(path.join(srcDir, 'data.ts'), output);
console.log('generated src/generated-tree-data.ts and synced src/data.ts from registry');
